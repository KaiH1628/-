      ...
      >
        <template slot-scope="scope">
          ...
          >
            ...
            ></el-image>
            <a href="#" target=" _blank" slot="reference" @click.prevent="goToTitleHandle(scope.row)">{{
                scope.row.title
              }}</a>
          </el-popover>
        </template>
      ...
      >
        <template slot-scope="scope">
          <el-tooltip class="item" effect="dark" content="编辑" placement="top" :hide-after="2000">
            <el-button type="primary" icon="el-icon-edit" circle size="mini" @click="handleClick(scope.row)"
            ></el-button>
          </el-tooltip>
  ...
  methods: {
    fetchData() {
      ...
      getBlog(this.currentPage, this.eachPage).then(({ data }) => {
        ...
        //  下面的if在删除文章的时候可能会触发，例如最后一页只有一条数据
    ...
    deleteBlog(blogInfo) {
      ...
      this.$confirm('删除该文章会将该文章下的评论一并删除, 是否继续?', '是否删除该文章', {
