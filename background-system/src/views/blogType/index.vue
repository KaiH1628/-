<template>
  <div class="app-container">
    ...
    <div style="margin-top: 15px;">
      <el-input placeholder="请输入要添加的分类，左边选择该分类的等级" v-model="input" class="input-with-select blogAddInput">
      ...
      <el-button type="primary" style="margin-left: 10px" @click="addBlogType">添加</el-button>
    </div>
    <!--    数据表格-->
    <el-table
      :data="data"
      ...
      >
        <template slot-scope="scope">
          <el-tooltip class="item" effect="dark" content="编辑" placement="top" :hide-after="2000">
            <el-button type="primary" icon="el-icon-edit" circle size="mini" @click="editBlogTypeHandle(scope.row)"
            ></el-button>
          </el-tooltip>
          <el-tooltip class="item" effect="dark" content="删除" placement="top" :hide-after="2000">
            <el-button type="danger" icon="el-icon-delete" circle size="mini" @click="deleteBlogTypeHandle(scope.row)"
            ></el-button>
          </el-tooltip>
        </template>
      </el-table-column>
...
export default {
  ...
  methods: {
    ...
    deleteBlogTypeHandle(blogTypeInfo) {
      ...
      this.$confirm('删除该分类后，该分类下所有文章将变为未分类状态, 是否继续?', '是否删除此文章分类', {
